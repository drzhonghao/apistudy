...
public abstract class AbstractWordConverter {
	...
	private static final class Structure implements Comparable<AbstractWordConverter.Structure> {
		final int end;
		...
		Structure(AbstractWordConverter.DeadFieldBoundaries deadFieldBoundaries, int start, int end) {
			this.start = start;
			this.end = end;
		...
		public int compareTo(AbstractWordConverter.Structure o) {
			return Integer.compare(start, o.start);
		}

		@Override
		public String toString() {
			return (((("Structure [" + (start)) + "; ") + (end)) + "): ") + (structure);
		}
	}
	...
	private static void addToStructures(List<AbstractWordConverter.Structure> structures, AbstractWordConverter.Structure structure) {
		for (Iterator<AbstractWordConverter.Structure> iterator = structures.iterator(); iterator.hasNext();) {
			AbstractWordConverter.Structure another = iterator.next();
	...
	protected boolean processCharacters(final HWPFDocumentCore wordDocument, final int currentTableLevel, final Range range, final Element block) {
		...
		List<AbstractWordConverter.Structure> structures = new LinkedList<>();
		if (wordDocument instanceof HWPFDocument) {
				...
				for (List<Bookmark> lists : rangeBookmarks.values()) {
					for (Bookmark bookmark : lists) {
						if (!(bookmarkStack.contains(bookmark)))
							AbstractWordConverter.addToStructures(structures, new AbstractWordConverter.Structure(bookmark));
			...
			for (int c = 0; c < (range.numCharacterRuns()); c++) {
				...
				if (((text == null) || ((text.length()) == 0)) || ((text.charAt(0)) != (AbstractWordConverter.FIELD_BEGIN_MARK)))
					continue;
				...
				if (aliveField != null) {
					AbstractWordConverter.addToStructures(structures, new AbstractWordConverter.Structure(aliveField));
				}else {
					int[] separatorEnd = tryDeadField_lookupFieldSeparatorEnd(wordDocument, range, c);
					if (separatorEnd != null) {
						AbstractWordConverter.addToStructures(structures, new AbstractWordConverter.Structure(new AbstractWordConverter.DeadFieldBoundaries(c, separatorEnd[0], separatorEnd[1]), characterRun.getStartOffset(), range.getCharacterRun(separatorEnd[1]).getEndOffset()));
						c = separatorEnd[1];
					}
				}
			}
		...
		for (AbstractWordConverter.Structure structure : structures) {
			...
			if ((structure.structure) instanceof Bookmark) {
				...
				if ((structure.structure) instanceof Field) {
					Field field = ((Field) (structure.structure));
					processField(((HWPFDocument) (wordDocument)), range, currentTableLevel, field, block);
				}else
					if ((structure.structure) instanceof AbstractWordConverter.DeadFieldBoundaries) {
						AbstractWordConverter.DeadFieldBoundaries boundaries = ((AbstractWordConverter.DeadFieldBoundaries) (structure.structure));
		...
		for (int c = 0; c < (range.numCharacterRuns()); c++) {
			...
			if ((wordDocument instanceof HWPFDocument) && (((HWPFDocument) (wordDocument)).getPicturesTable().hasPicture(characterRun))) {
				...
				processImage(block, ((characterRun.text().charAt(0)) == 1), picture);
				continue;
			}
			String text = characterRun.text();
			if (text.isEmpty())
			...
			if (characterRun.isSpecialCharacter()) {
				if (((text.charAt(0)) == (AbstractWordConverter.SPECCHAR_AUTONUMBERED_FOOTNOTE_REFERENCE)) && (wordDocument instanceof HWPFDocument)) {
					HWPFDocument doc = ((HWPFDocument) (wordDocument));
					processNoteAnchor(doc, characterRun, block);
					continue;
				}
				if (((text.charAt(0)) == (AbstractWordConverter.SPECCHAR_DRAWN_OBJECT)) && (wordDocument instanceof HWPFDocument)) {
					HWPFDocument doc = ((HWPFDocument) (wordDocument));
			...
			if ((text.charAt(0)) == (AbstractWordConverter.FIELD_BEGIN_MARK)) {
				if (wordDocument instanceof HWPFDocument) {
					Field aliveField = ((HWPFDocument) (wordDocument)).getFields().getFieldByStartOffset(FieldsDocumentPart.MAIN, characterRun.getStartOffset());
					if (aliveField != null) {
			...
			if ((text.charAt(0)) == (AbstractWordConverter.FIELD_SEPARATOR_MARK)) {
				continue;
			}
			if ((text.charAt(0)) == (AbstractWordConverter.FIELD_END_MARK)) {
				continue;
			}
			if (((characterRun.isSpecialCharacter()) || (characterRun.isObj())) || (characterRun.isOle2())) {
				continue;
			}
			if ((text.endsWith("\r")) || (((text.charAt(((text.length()) - 1))) == (AbstractWordConverter.BEL_MARK)) && (currentTableLevel != (Integer.MIN_VALUE))))
...

			{
				...
				for (char charChar : text.toCharArray()) {
					if (charChar == 11) {
						...
						if (charChar == 30) {
							stringBuilder.append(AbstractWordConverter.UNICODECHAR_NONBREAKING_HYPHEN);
						}else
							if (charChar == 31) {
								stringBuilder.append(AbstractWordConverter.UNICODECHAR_ZERO_WIDTH_SPACE);
							}else
								if ((((charChar >= 32) || (charChar == 9)) || (charChar == 10)) || (charChar == 13)) {
	...
	protected void processDocumentPart(HWPFDocumentCore wordDocument, final Range range) {
			...
			case 37 :
				{
					...
					if (firstSubrange != null) {
						...
						Matcher matcher = AbstractWordConverter.PATTERN_PAGEREF.matcher(formula);
						if (matcher.find()) {
							String pageref = matcher.group(1);
							processPageref(wordDocument, currentBlock, field.secondSubrange(parentRange), currentTableLevel, pageref);
							return;
						}
					}
					break;
			...
			case 58 :
				{
					...
					if (separator.isOle2()) {
						boolean processed = processOle2(wordDocument, separator, currentBlock);
						if (!processed) {
							processCharacters(wordDocument, currentTableLevel, field.secondSubrange(parentRange), currentBlock);
						}
						return;
					}
					break;
			...
			case 83 :
				{
					...
					if ((values != null) && ((values.length) > 0)) {
						processDropDownList(currentBlock, cr, values, (defIndex == null ? -1 : defIndex.intValue()));
						return;
					}
					break;
				}
			case 88 :
				{
					...
					if (firstSubrange != null) {
						String formula = firstSubrange.text();
						Matcher matcher = AbstractWordConverter.PATTERN_HYPERLINK_EXTERNAL.matcher(formula);
						if (matcher.matches()) {
							String hyperlink = matcher.group(1);
							processHyperlink(wordDocument, currentBlock, field.secondSubrange(parentRange), currentTableLevel, hyperlink);
							return;
						}
						...
						if (matcher.matches()) {
							...
							processPageref(wordDocument, currentBlock, textRange, currentTableLevel, hyperlink);
							return;
						}
					}
					break;
	...
	protected abstract void processImageWithoutPicturesManager(Element currentBlock, boolean inlined, Picture picture);
	...
	protected void processNoteAnchor(HWPFDocument doc, CharacterRun characterRun, final Element block) {
		...
		}
		{
			...
			int noteIndex = endnotes.getNoteIndexByAnchorPosition(characterRun.getStartOffset());
			if (noteIndex != (-1)) {
	...
	private boolean processOle2(HWPFDocument doc, CharacterRun characterRun, Element block) {
		...
		if (entry == null) {
			AbstractWordConverter.logger.log(POILogger.WARN, "Referenced OLE2 object '", Integer.valueOf(characterRun.getPicOffset()), "' not found in ObjectPool");
			return false;
		}
		try {
			return processOle2(doc, block, entry);
		} catch (Exception exc) {
			AbstractWordConverter.logger.log(POILogger.WARN, "Unable to convert internal OLE2 object '", Integer.valueOf(characterRun.getPicOffset()), "': ", exc, exc);
			return false;
		}
	}
	...
	protected int tryDeadField(HWPFDocumentCore wordDocument, Range range, int currentTableLevel, int beginMark, Element currentBlock) {
		int[] separatorEnd = tryDeadField_lookupFieldSeparatorEnd(wordDocument, range, beginMark);
		if (separatorEnd == null)
			return beginMark;

		processDeadField(wordDocument, currentBlock, range, currentTableLevel, beginMark, separatorEnd[0], separatorEnd[1]);
	...
	private int[] tryDeadField_lookupFieldSeparatorEnd(HWPFDocumentCore wordDocument, Range range, int beginMark) {
		...
		for (int c = beginMark + 1; c < (range.numCharacterRuns()); c++) {
			...
			if (firstByte == (AbstractWordConverter.FIELD_BEGIN_MARK)) {
			...
			if (firstByte == (AbstractWordConverter.FIELD_SEPARATOR_MARK)) {
				if (separatorMark != (-1)) {
					return null;
				}
				separatorMark = c;
			...
			if (firstByte == (AbstractWordConverter.FIELD_END_MARK)) {
				if (endMark != (-1)) {
					return null;
				}
				endMark = c;
