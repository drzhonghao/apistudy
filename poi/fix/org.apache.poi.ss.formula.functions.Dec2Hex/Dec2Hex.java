...
public final class Dec2Hex implements FreeRefFunction {
	...
	public ValueEval evaluate(int srcRowIndex, int srcColumnIndex, ValueEval number, ValueEval places) {
		...
		if (((number1.longValue()) < (Dec2Hex.MIN_VALUE)) || ((number1.longValue()) > (Dec2Hex.MAX_VALUE))) {
			return ErrorEval.NUM_ERROR;
		}
		int placesNumber = 0;
		if (number1 < 0) {
			placesNumber = Dec2Hex.DEFAULT_PLACES_VALUE;
		}else
			if (places != null) {
