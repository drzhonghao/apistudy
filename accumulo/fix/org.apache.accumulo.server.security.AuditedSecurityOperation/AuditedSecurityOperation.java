...
public class AuditedSecurityOperation extends SecurityOperation {
	...
	public static final Logger audit = Logger.getLogger(AuditedSecurityOperation.AUDITLOG);
	...
	public static synchronized SecurityOperation getInstance(AccumuloServerContext context) {
		return AuditedSecurityOperation.getInstance(context, false);
	}
	...
	private boolean shouldAudit(TCredentials credentials, String tableId) {
		return (((AuditedSecurityOperation.audit.isInfoEnabled()) || (AuditedSecurityOperation.audit.isEnabledFor(Level.WARN))) && (!(tableId.equals(MetadataTable.ID)))) && (shouldAudit(credentials));
	}
	...
	private void audit(TCredentials credentials, ThriftSecurityException ex, String template, Object... args) {
		AuditedSecurityOperation.audit.warn(((((("operation: failed; user: " + (credentials.getPrincipal())) + "; ") + (String.format(template, args))) + "; exception: ") + (ex.toString())));
	}

	private void audit(TCredentials credentials, String template, Object... args) {
		if ((AuditedSecurityOperation.audit.isInfoEnabled()) && (shouldAudit(credentials))) {
			AuditedSecurityOperation.audit.info(((("operation: success; user: " + (credentials.getPrincipal())) + ": ") + (String.format(template, args))));
		}
	}
	...
	private void audit(TCredentials credentials, boolean permitted, String template, Object... args) {
		if ((AuditedSecurityOperation.audit.isInfoEnabled()) && (shouldAudit(credentials))) {
			...
			AuditedSecurityOperation.audit.info(((((((("operation: " + prefix) + "; user: ") + (credentials.getPrincipal())) + "; client: ") + (TServerUtils.clientAddress.get())) + "; ") + (String.format(template, args))));
		}
	}
	...
	private static List<String> truncate(Collection<?> list) {
		...
		int i = 0;
		for (Object obj : list) {
			if ((i++) > (AuditedSecurityOperation.MAX_ELEMENTS_TO_LOG)) {
				result.add(((" and " + ((list.size()) - (AuditedSecurityOperation.MAX_ELEMENTS_TO_LOG))) + " more "));
				break;
			}
	...
	public boolean canScan(TCredentials credentials, String tableId, String namespaceId, TRange range, List<TColumn> columns, List<IterInfo> ssiList, Map<String, Map<String, String>> ssio, List<ByteBuffer> authorizations) throws ThriftSecurityException {
		if (shouldAudit(credentials, tableId)) {
			...
			List<String> convertedColumns = AuditedSecurityOperation.truncate(Translator.translate(columns, new Translator.TColumnTranslator()));
			String tableName = getTableName(tableId);
			try {
				boolean canScan = super.canScan(credentials, tableId, namespaceId);
				audit(credentials, canScan, AuditedSecurityOperation.CAN_SCAN_AUDIT_TEMPLATE, tableName, AuditedSecurityOperation.getAuthString(authorizations), convertedRange, convertedColumns, ssiList, ssio);
				return canScan;
			} catch (ThriftSecurityException ex) {
				audit(credentials, ex, AuditedSecurityOperation.CAN_SCAN_AUDIT_TEMPLATE, AuditedSecurityOperation.getAuthString(authorizations), tableId, convertedRange, convertedColumns, ssiList, ssio);
				throw ex;
			}
		}else {
	...
	public boolean canScan(TCredentials credentials, String tableId, String namespaceId, Map<TKeyExtent, List<TRange>> tbatch, List<TColumn> tcolumns, List<IterInfo> ssiList, Map<String, Map<String, String>> ssio, List<ByteBuffer> authorizations) throws ThriftSecurityException {
		if (shouldAudit(credentials, tableId)) {
			...
			for (Map.Entry<KeyExtent, List<Range>> entry : convertedBatch.entrySet()) {
				truncated.put(entry.getKey(), AuditedSecurityOperation.truncate(entry.getValue()));
			}
			List<Column> convertedColumns = Translator.translate(tcolumns, new Translator.TColumnTranslator());
			String tableName = getTableName(tableId);
			try {
				boolean canScan = super.canScan(credentials, tableId, namespaceId);
				audit(credentials, canScan, AuditedSecurityOperation.CAN_SCAN_BATCH_AUDIT_TEMPLATE, tableName, AuditedSecurityOperation.getAuthString(authorizations), truncated, convertedColumns, ssiList, ssio);
				return canScan;
			} catch (ThriftSecurityException ex) {
				audit(credentials, ex, AuditedSecurityOperation.CAN_SCAN_BATCH_AUDIT_TEMPLATE, AuditedSecurityOperation.getAuthString(authorizations), tableId, truncated, convertedColumns, ssiList, ssio);
				throw ex;
			}
		}else {
			return super.canScan(credentials, tableId, namespaceId);
	...
	public void changeAuthorizations(TCredentials credentials, String user, Authorizations authorizations) throws ThriftSecurityException {
		try {
			...
			audit(credentials, AuditedSecurityOperation.CHANGE_AUTHORIZATIONS_AUDIT_TEMPLATE, user, authorizations);
		} catch (ThriftSecurityException ex) {
			audit(credentials, ex, AuditedSecurityOperation.CHANGE_AUTHORIZATIONS_AUDIT_TEMPLATE, user, authorizations);
			throw ex;
		}
	}
	...
	public void changePassword(TCredentials credentials, Credentials newInfo) throws ThriftSecurityException {
		try {
			...
			audit(credentials, AuditedSecurityOperation.CHANGE_PASSWORD_AUDIT_TEMPLATE, newInfo.getPrincipal());
		} catch (ThriftSecurityException ex) {
			audit(credentials, ex, AuditedSecurityOperation.CHANGE_PASSWORD_AUDIT_TEMPLATE, newInfo.getPrincipal());
			throw ex;
		}
	}
	...
	public void createUser(TCredentials credentials, Credentials newUser, Authorizations authorizations) throws ThriftSecurityException {
		try {
			...
			audit(credentials, AuditedSecurityOperation.CREATE_USER_AUDIT_TEMPLATE, newUser.getPrincipal(), authorizations);
		} catch (ThriftSecurityException ex) {
			audit(credentials, ex, AuditedSecurityOperation.CREATE_USER_AUDIT_TEMPLATE, newUser.getPrincipal(), authorizations);
			throw ex;
		}
	}
	...
	public boolean canCreateTable(TCredentials c, String tableName, String namespaceId) throws ThriftSecurityException {
		try {
			...
			audit(c, result, AuditedSecurityOperation.CAN_CREATE_TABLE_AUDIT_TEMPLATE, tableName);
			return result;
		} catch (ThriftSecurityException ex) {
			audit(c, ex, AuditedSecurityOperation.CAN_CREATE_TABLE_AUDIT_TEMPLATE, tableName);
			throw ex;
		}
	}
	...
	public boolean canDeleteTable(TCredentials c, String tableId, String namespaceId) throws ThriftSecurityException {
		...
		try {
			...
			audit(c, result, AuditedSecurityOperation.CAN_DELETE_TABLE_AUDIT_TEMPLATE, tableName, tableId);
			return result;
		} catch (ThriftSecurityException ex) {
			audit(c, ex, AuditedSecurityOperation.CAN_DELETE_TABLE_AUDIT_TEMPLATE, tableName, tableId);
			throw ex;
		}
	}
	...
	public boolean canRenameTable(TCredentials c, String tableId, String oldTableName, String newTableName, String namespaceId) throws ThriftSecurityException {
		try {
			...
			audit(c, result, AuditedSecurityOperation.CAN_RENAME_TABLE_AUDIT_TEMPLATE, oldTableName, newTableName);
			return result;
		} catch (ThriftSecurityException ex) {
			audit(c, ex, AuditedSecurityOperation.CAN_RENAME_TABLE_AUDIT_TEMPLATE, oldTableName, newTableName);
			throw ex;
		}
	}
	...
	public boolean canSplitTablet(TCredentials credentials, String table, String namespaceId) throws ThriftSecurityException {
		try {
			...
			audit(credentials, result, AuditedSecurityOperation.CAN_SPLIT_TABLE_AUDIT_TEMPLATE, table, namespaceId);
			return result;
		} catch (ThriftSecurityException ex) {
	...
	public boolean canPerformSystemActions(TCredentials credentials) throws ThriftSecurityException {
		try {
			...
			audit(credentials, result, AuditedSecurityOperation.CAN_PERFORM_SYSTEM_ACTION_AUDIT_TEMPLATE, credentials.getPrincipal());
			return result;
		} catch (ThriftSecurityException ex) {
	...
	public boolean canFlush(TCredentials c, String tableId, String namespaceId) throws ThriftSecurityException {
		try {
			...
			audit(c, result, AuditedSecurityOperation.CAN_FLUSH_TABLE_AUDIT_TEMPLATE, tableId, namespaceId);
			return result;
		} catch (ThriftSecurityException ex) {
	...
	public boolean canAlterTable(TCredentials c, String tableId, String namespaceId) throws ThriftSecurityException {
		try {
			...
			audit(c, result, AuditedSecurityOperation.CAN_ALTER_TABLE_AUDIT_TEMPLATE, tableId, namespaceId);
			return result;
		} catch (ThriftSecurityException ex) {
	...
	public boolean canCloneTable(TCredentials c, String tableId, String tableName, String destinationNamespaceId, String sourceNamespaceId) throws ThriftSecurityException {
		...
		try {
			...
			audit(c, result, AuditedSecurityOperation.CAN_CLONE_TABLE_AUDIT_TEMPLATE, oldTableName, tableName);
			return result;
		} catch (ThriftSecurityException ex) {
			audit(c, ex, AuditedSecurityOperation.CAN_CLONE_TABLE_AUDIT_TEMPLATE, oldTableName, tableName);
			throw ex;
		}
	}
	...
	public boolean canDeleteRange(TCredentials c, String tableId, String tableName, Text startRow, Text endRow, String namespaceId) throws ThriftSecurityException {
		try {
			...
			audit(c, result, AuditedSecurityOperation.CAN_DELETE_RANGE_AUDIT_TEMPLATE, tableName, startRow.toString(), endRow.toString());
			return result;
		} catch (ThriftSecurityException ex) {
			audit(c, ex, AuditedSecurityOperation.CAN_DELETE_RANGE_AUDIT_TEMPLATE, tableName, startRow.toString(), endRow.toString());
			throw ex;
		}
	}
	...
	public boolean canBulkImport(TCredentials c, String tableId, String tableName, String dir, String failDir, String namespaceId) throws ThriftSecurityException {
		try {
			...
			audit(c, result, AuditedSecurityOperation.CAN_BULK_IMPORT_AUDIT_TEMPLATE, tableName, dir, failDir);
			return result;
		} catch (ThriftSecurityException ex) {
			audit(c, ex, AuditedSecurityOperation.CAN_BULK_IMPORT_AUDIT_TEMPLATE, tableName, dir, failDir);
			throw ex;
		}
	}
	...
	public boolean canCompact(TCredentials c, String tableId, String namespaceId) throws ThriftSecurityException {
		try {
			...
			audit(c, result, AuditedSecurityOperation.CAN_COMPACT_TABLE_AUDIT_TEMPLATE, tableId, namespaceId);
			return result;
		} catch (ThriftSecurityException ex) {
	...
	public boolean canChangeAuthorizations(TCredentials c, String user) throws ThriftSecurityException {
		try {
			...
			audit(c, result, AuditedSecurityOperation.CAN_CHANGE_AUTHORIZATIONS_AUDIT_TEMPLATE, user);
			return result;
		} catch (ThriftSecurityException ex) {
	...
	public boolean canChangePassword(TCredentials c, String user) throws ThriftSecurityException {
		try {
			...
			audit(c, result, AuditedSecurityOperation.CAN_CHANGE_PASSWORD_AUDIT_TEMPLATE, user);
			return result;
		} catch (ThriftSecurityException ex) {
	...
	public boolean canCreateUser(TCredentials c, String user) throws ThriftSecurityException {
		try {
			...
			audit(c, result, AuditedSecurityOperation.CAN_CREATE_USER_AUDIT_TEMPLATE, user);
			return result;
		} catch (ThriftSecurityException ex) {
	...
	public boolean canDropUser(TCredentials c, String user) throws ThriftSecurityException {
		try {
			...
			audit(c, result, AuditedSecurityOperation.CAN_DROP_USER_AUDIT_TEMPLATE, user);
			return result;
		} catch (ThriftSecurityException ex) {
	...
	public boolean canGrantSystem(TCredentials c, String user, SystemPermission sysPerm) throws ThriftSecurityException {
		try {
			...
			audit(c, result, AuditedSecurityOperation.CAN_GRANT_SYSTEM_AUDIT_TEMPLATE, user, sysPerm);
			return result;
		} catch (ThriftSecurityException ex) {
	...
	public boolean canGrantTable(TCredentials c, String user, String table, String namespaceId) throws ThriftSecurityException {
		try {
			...
			audit(c, result, AuditedSecurityOperation.CAN_GRANT_TABLE_AUDIT_TEMPLATE, user, table, namespaceId);
			return result;
		} catch (ThriftSecurityException ex) {
	...
	public boolean canRevokeSystem(TCredentials c, String user, SystemPermission sysPerm) throws ThriftSecurityException {
		try {
			...
			audit(c, result, AuditedSecurityOperation.CAN_REVOKE_SYSTEM_AUDIT_TEMPLATE, user, sysPerm);
			return result;
		} catch (ThriftSecurityException ex) {
	...
	public boolean canRevokeTable(TCredentials c, String user, String table, String namespaceId) throws ThriftSecurityException {
		try {
			...
			audit(c, result, AuditedSecurityOperation.CAN_REVOKE_TABLE_AUDIT_TEMPLATE, user, table, namespaceId);
			return result;
		} catch (ThriftSecurityException ex) {
	...
	public boolean canImport(TCredentials credentials, String tableName, String importDir, String namespaceId) throws ThriftSecurityException {
		try {
			...
			audit(credentials, result, AuditedSecurityOperation.CAN_IMPORT_AUDIT_TEMPLATE, tableName, importDir);
			return result;
		} catch (ThriftSecurityException ex) {
			audit(credentials, ex, AuditedSecurityOperation.CAN_IMPORT_AUDIT_TEMPLATE, tableName, importDir);
			throw ex;
		}
	}
	...
	public boolean canExport(TCredentials credentials, String tableId, String tableName, String exportDir, String namespaceId) throws ThriftSecurityException {
		try {
			...
			audit(credentials, result, AuditedSecurityOperation.CAN_EXPORT_AUDIT_TEMPLATE, tableName, exportDir);
			return result;
		} catch (ThriftSecurityException ex) {
			audit(credentials, ex, AuditedSecurityOperation.CAN_EXPORT_AUDIT_TEMPLATE, tableName, exportDir);
			throw ex;
		}
	}
	...
	public void dropUser(TCredentials credentials, String user) throws ThriftSecurityException {
		try {
			...
			audit(credentials, AuditedSecurityOperation.DROP_USER_AUDIT_TEMPLATE, user);
		} catch (ThriftSecurityException ex) {
			audit(credentials, ex, AuditedSecurityOperation.DROP_USER_AUDIT_TEMPLATE, user);
			throw ex;
		}
	}
	...
	public void grantSystemPermission(TCredentials credentials, String user, SystemPermission permission) throws ThriftSecurityException {
		try {
			...
			audit(credentials, AuditedSecurityOperation.GRANT_SYSTEM_PERMISSION_AUDIT_TEMPLATE, permission, user);
		} catch (ThriftSecurityException ex) {
			audit(credentials, ex, AuditedSecurityOperation.GRANT_SYSTEM_PERMISSION_AUDIT_TEMPLATE, permission, user);
			throw ex;
		}
	}
	...
	public void grantTablePermission(TCredentials credentials, String user, String tableId, TablePermission permission, String namespaceId) throws ThriftSecurityException {
		...
		try {
			...
			audit(credentials, AuditedSecurityOperation.GRANT_TABLE_PERMISSION_AUDIT_TEMPLATE, permission, tableName, user);
		} catch (ThriftSecurityException ex) {
			audit(credentials, ex, AuditedSecurityOperation.GRANT_TABLE_PERMISSION_AUDIT_TEMPLATE, permission, tableName, user);
			throw ex;
		}
	}
	...
	public void revokeSystemPermission(TCredentials credentials, String user, SystemPermission permission) throws ThriftSecurityException {
		try {
			...
			audit(credentials, AuditedSecurityOperation.REVOKE_SYSTEM_PERMISSION_AUDIT_TEMPLATE, permission, user);
		} catch (ThriftSecurityException ex) {
			audit(credentials, ex, AuditedSecurityOperation.REVOKE_SYSTEM_PERMISSION_AUDIT_TEMPLATE, permission, user);
			throw ex;
		}
	}
	...
	public void revokeTablePermission(TCredentials credentials, String user, String tableId, TablePermission permission, String namespaceId) throws ThriftSecurityException {
		...
		try {
			...
			audit(credentials, AuditedSecurityOperation.REVOKE_TABLE_PERMISSION_AUDIT_TEMPLATE, permission, tableName, user);
		} catch (ThriftSecurityException ex) {
			audit(credentials, ex, AuditedSecurityOperation.REVOKE_TABLE_PERMISSION_AUDIT_TEMPLATE, permission, tableName, user);
			throw ex;
		}
	}
	...
	public boolean hasSystemPermission(TCredentials credentials, String user, SystemPermission permission) throws ThriftSecurityException {
		try {
			...
			audit(credentials, result, AuditedSecurityOperation.HAS_SYSTEM_PERMISSION_AUDIT_TEMPLATE, permission, user);
			return result;
		} catch (ThriftSecurityException ex) {
	...
	public boolean canOnlineOfflineTable(TCredentials credentials, String tableId, FateOperation op, String namespaceId) throws ThriftSecurityException {
		...
		try {
			...
			audit(credentials, result, AuditedSecurityOperation.CAN_ONLINE_OFFLINE_TABLE_AUDIT_TEMPLATE, operation, tableName, tableId);
			return result;
		} catch (ThriftSecurityException ex) {
			audit(credentials, ex, AuditedSecurityOperation.CAN_ONLINE_OFFLINE_TABLE_AUDIT_TEMPLATE, operation, tableName, tableId);
			throw ex;
		}
	}
	...
	public boolean canMerge(TCredentials c, String tableId, String namespaceId) throws ThriftSecurityException {
		try {
			...
			audit(c, result, AuditedSecurityOperation.CAN_MERGE_TABLE_AUDIT_TEMPLATE, tableId, namespaceId);
			return result;
		} catch (ThriftSecurityException ex) {
	...
	protected void authenticate(TCredentials credentials) throws ThriftSecurityException {
		try {
			...
			audit(credentials, true, AuditedSecurityOperation.AUTHENICATE_AUDIT_TEMPLATE);
		} catch (ThriftSecurityException e) {
			audit(credentials, false, AuditedSecurityOperation.AUTHENICATE_AUDIT_TEMPLATE);
			throw e;
		}
	}
	...
	public boolean canObtainDelegationToken(TCredentials credentials) throws ThriftSecurityException {
		try {
			...
			audit(credentials, result, AuditedSecurityOperation.DELEGATION_TOKEN_AUDIT_TEMPLATE);
			return result;
		} catch (ThriftSecurityException e) {
			audit(credentials, false, AuditedSecurityOperation.DELEGATION_TOKEN_AUDIT_TEMPLATE);
			throw e;
		}
	}
}
