...
public final class Buffer implements Appendable , CharSequence {
	...
	protected final Buffer.Format format;
	...
	public Buffer(Buffer.Format format) {
		this(format, 1024);
	}

	public Buffer(Buffer.Format format, int size) {
		this.format = format;
		this.capacity = size;
	...
	public Buffer format(Object[] array) throws IOException {
		...
		if ((format) == (Buffer.Format.Syslog)) {
			buffer[((position)++)] = '\\';
		}
		buffer[((position)++)] = ']';
		return this;
	...
	public Buffer format(Subject subject) throws IOException {
		...
		for (Principal p : subject.getPrincipals()) {
			if (p instanceof UserPrincipal) {
				...
				if (p instanceof ClientPrincipal) {
					cp = p.getName();
				}
			...
			switch (c) {
				...
				default :
					if (((c < ' ') || ((c >= '\u0080') && (c < '\u00a0'))) || ((c >= '\u2000') && (c < '\u2100'))) {
						d[(position++)] = '\\';
						d[(position++)] = 'u';
						d[(position++)] = Buffer.HEX_DIGITS[(c >> 12)];
						d[(position++)] = Buffer.HEX_DIGITS[((c >> 8) & 15)];
						d[(position++)] = Buffer.HEX_DIGITS[((c >> 4) & 15)];
						d[(position++)] = Buffer.HEX_DIGITS[(c & 15)];
					}else {
						d[(position++)] = c;
					}
					break;
