...
import static org.apache.karaf.service.guard.tools.ACLConfigurationParser.Specificity.NO_MATCH;
...
public class SecuredSessionFactoryImpl extends SessionFactoryImpl implements ConfigurationListener {
	...
	private static final String CONFIGURATION_FILTER = ((("(" + (Constants.SERVICE_PID)) + "=") + (SecuredSessionFactoryImpl.PROXY_COMMAND_ACL_PID_PREFIX)) + "*)";
	...
	protected Object invoke(CommandSessionImpl session, Object target, String name, List<Object> args) throws Exception {
		checkSecurity(SecuredSessionFactoryImpl.SHELL_SCOPE, SecuredSessionFactoryImpl.SHELL_INVOKE, Arrays.asList(target, name, args));
		return super.invoke(session, target, name, args);
	}

	@Override
	protected Path redirect(CommandSessionImpl session, Path path, int mode) {
		checkSecurity(SecuredSessionFactoryImpl.SHELL_SCOPE, SecuredSessionFactoryImpl.SHELL_REDIRECT, Arrays.asList(path, mode));
		...
		if (config != null) {
			...
			if (roles.isEmpty()) {
				...
				for (String role : roles) {
					if (SecuredSessionFactoryImpl.currentUserHasRole(role)) {
						visible = true;
					}
				}
			}
				...
				if ((content != null) && ("org.apache.felix.gogo.runtime.Closure".equals(content.getClass().getName()))) {
					...
					if (index > 0) {
						...
						if (securityCmd instanceof SecuredCommand) {
							if ((((SecuredCommand) (securityCmd)).getScope().equals(scope)) && (((SecuredCommand) (securityCmd)).getName().equals(name))) {
	...
	public void configurationEvent(ConfigurationEvent event) {
		if (!(event.getPid().startsWith(SecuredSessionFactoryImpl.PROXY_COMMAND_ACL_PID_PREFIX)))
			return;
		...
		try {
			...
			switch (event.getType()) {
				case ConfigurationEvent.CM_DELETED :
					removeScopeConfig(event.getPid().substring(SecuredSessionFactoryImpl.PROXY_COMMAND_ACL_PID_PREFIX.length()));
					break;
			...
			SecuredSessionFactoryImpl.LOGGER.error("Problem processing Configuration Event {}", event, e);
		}
	}

	private void addScopeConfig(Configuration config) {
		if (!(config.getPid().startsWith(SecuredSessionFactoryImpl.PROXY_COMMAND_ACL_PID_PREFIX))) {
			return;
		}
		String scope = config.getPid().substring(SecuredSessionFactoryImpl.PROXY_COMMAND_ACL_PID_PREFIX.length());
		if ((scope.indexOf('.')) >= 0) {
			return;
		}
		scope = scope.trim();
	...
	protected void update(ConfigurationAdmin prev, ConfigurationAdmin configAdmin) {
		try {
			Configuration[] configs = configAdmin.listConfigurations(SecuredSessionFactoryImpl.CONFIGURATION_FILTER);
			if (configs != null) {
