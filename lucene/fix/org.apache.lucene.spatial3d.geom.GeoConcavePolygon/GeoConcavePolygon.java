...
class GeoConcavePolygon {
	...
	public GeoConcavePolygon(final PlanetModel planetModel, final List<GeoPoint> pointList, final BitSet internalEdgeFlags, final boolean returnEdgeInternal) {
		this(planetModel, pointList, null, internalEdgeFlags, returnEdgeInternal);
	}

	public GeoConcavePolygon(final PlanetModel planetModel, final List<GeoPoint> pointList, final List<GeoPolygon> holes, final BitSet internalEdgeFlags, final boolean returnEdgeInternal) {
		this.points = pointList;
		if ((holes != null) && ((holes.size()) == 0)) {
			this.holes = null;
		}else {
	...
	public GeoConcavePolygon(final PlanetModel planetModel, final double startLatitude, final double startLongitude) {
		this(planetModel, startLatitude, startLongitude, null);
	}

	public GeoConcavePolygon(final PlanetModel planetModel, final double startLatitude, final double startLongitude, final List<GeoPolygon> holes) {
	...
	public void done(final boolean isInternalReturnEdge) {
		...
		for (int edgeIndex = 0; edgeIndex < (edges.length); edgeIndex++) {
			...
			eitherBounds.put(edge, new GeoConcavePolygon.EitherBound(invertedEdges[bound1Index], invertedEdges[bound2Index]));
